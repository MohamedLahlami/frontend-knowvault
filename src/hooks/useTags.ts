...
export const useTags = () => {
    ...
    const fetchTags = async (pageToFetch = page) => {
        ...
        setLoading(true);
        try {
            const data = await getShelfTags(auth.user.access_token /*, pageToFetch, size? */);
            setTags(data);
            setError(null);
        } catch (err) {
    ...
    const handleCreateShelfTag = async (label: string, type: "SHELF"): Promise<Tag | null> => {
        if (!auth.isAuthenticated || !auth.user) {
            setError("Utilisateur non authentifié");
            return null;
        }
