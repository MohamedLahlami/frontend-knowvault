...
export default function Dashboard() {
  ...
  const recentBooks = [
    { title: "Guide de démarrage React", author: "Documentation", updatedAt: "Il y a 2 heures" },
    { title: "Bonnes pratiques JavaScript", author: "Équipe Dev", updatedAt: "Hier" },
    { title: "Architecture système", author: "Tech Lead", updatedAt: "Il y a 3 jours" },
  ]

  const recentShelves = [
    { title: "Documentation technique", bookCount: 12, color: "bg-blue-100 text-blue-800" },
    { title: "Guides utilisateur", bookCount: 8, color: "bg-green-100 text-green-800" },
    { title: "Procédures", bookCount: 4, color: "bg-purple-100 text-purple-800" },
  ]
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Les derniers contenus mis à jour
            </CardDescription>
          </CardHeader>
            ...
            {recentBooks.map((book, index) => (
              <div key={index} className="flex items-start justify-between p-3 rounded-lg border hover:bg-muted/50 transition-colors">
                <div className="space-y-1">
                  <Link to={`/books/${index + 1}`} className="font-medium text-foreground hover:text-primary transition-colors">
                    {book.title}
                  </Link>
          ...
          <CardContent className="space-y-4">
            {recentShelves.map((shelf, index) => (
              <div key={index} className="flex items-center justify-between p-3 rounded-lg border hover:bg-muted/50 transition-colors">
                <div className="space-y-1">
                  <Link to={`/shelves/${index + 1}`} className="font-medium text-foreground hover:text-primary transition-colors">
                    {shelf.title}
                  </Link>
